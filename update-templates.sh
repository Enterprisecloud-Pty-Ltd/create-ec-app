...
find "$TEMPLATES_DIR" -maxdepth 10 -type f -name "package.patch.json" | while read -r patch_file; do
    ...
    (cd "$dir" && run_ncu -u --target minor --packageFile "package.patch.json" 2>/dev/null) || echo "    Warning: ncu update skipped for $patch_file"
done
...
apply_patches() {
    find "$TEMPLATES_DIR" -maxdepth 10 -type f -name "package.patch.json" | while read -r patch_file; do
        ...
        if [ -f "$package_file" ]; then
            ...
            echo "  Skipping layer patch without local package.json: $patch_file"
        fi
    done
}
...
find "$TEMPLATES_DIR" -maxdepth 10 -type f -name "package.json" ! -path "*/node_modules/*" | while read -r pkg_file; do
    ...
    (cd "$dir" && run_ncu -u --target minor 2>/dev/null) || echo "    Warning: ncu update skipped for $pkg_file"
done
